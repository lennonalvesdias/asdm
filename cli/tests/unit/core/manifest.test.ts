...
const MANIFEST: AsdmManifest = {
  ...
  assets: {
    'agents/code-reviewer.md': { sha256: 'abc123' + 'a'.repeat(58), size: 1000, version: '1.0.0' },
    'agents/tdd-guide.md': { sha256: 'def456' + 'b'.repeat(58), size: 1200, version: '1.0.0' },
    'skills/react-best-practices/SKILL.md': { sha256: 'ghi789' + 'c'.repeat(58), size: 800, version: '1.0.0' },
  },
}

describe('diffManifest', () => {
  it('marks all as added when no local files', () => {
    const diff = diffManifest(MANIFEST, {}, ['agents/code-reviewer.md', 'agents/tdd-guide.md'])
    expect(diff.added).toContain('agents/code-reviewer.md')
    expect(diff.added).toContain('agents/tdd-guide.md')
  ...
  it('marks as unchanged when sha matches', () => {
    const localShas = {
      'agents/code-reviewer.md': 'abc123' + 'a'.repeat(58),
    }
    const diff = diffManifest(MANIFEST, localShas, ['agents/code-reviewer.md'])
    expect(diff.unchanged).toContain('agents/code-reviewer.md')
  ...
  it('marks as updated when sha differs', () => {
    const localShas = {
      'agents/code-reviewer.md': 'old-sha-' + '0'.repeat(56),
    }
    const diff = diffManifest(MANIFEST, localShas, ['agents/code-reviewer.md'])
    expect(diff.updated).toContain('agents/code-reviewer.md')
  })
})
...
describe('getProfileAssetPaths', () => {
  it('returns paths for agents', () => {
    ...
    expect(paths).toContain('agents/tdd-guide.md')
  })

  it('returns paths for skills', () => {
    const paths = getProfileAssetPaths(MANIFEST, [], ['react-best-practices'], [])
    expect(paths).toContain('skills/react-best-practices/SKILL.md')
  })
